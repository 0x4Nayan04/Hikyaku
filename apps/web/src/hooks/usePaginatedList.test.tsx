...
function Harness({ pageSize = 25, fetchPage, fallbackError = 'Failed to load', enabled, queryKey }: Props) {
...
const page = (ids: number[], total: number, offset: number): Paginated<Item> => ({
  data: ids.map((id) => ({ id })),
  total,
  limit: 25,
  offset,
...
describe('usePaginatedList', () => {
  ...
  it('navigates between pages', async () => {
    const fetchPage = vi.fn<FetchPage>()
      .mockResolvedValueOnce(page([1, 2, 3, 4, 5], 30, 0))
      .mockResolvedValueOnce(page([26, 27, 28, 29, 30], 30, 25))
    await renderHarness({ fetchPage })
  ...
  it('corrects the offset when the final page disappears after a mutation', async () => {
    const fetchPage = vi.fn<FetchPage>()
      .mockResolvedValueOnce(page([1, 2, 3, 4, 5], 30, 0))
      .mockResolvedValueOnce(page([26, 27, 28, 29, 30], 30, 25))
      .mockResolvedValueOnce(page([26, 27, 28, 29, 30], 25, 25))
      .mockResolvedValueOnce(page([1, 2], 25, 0))
    await renderHarness({ fetchPage })
  ...
  it('reports a failed refresh without discarding the loaded list', async () => {
    const fetchPage = vi.fn<FetchPage>()
      .mockResolvedValueOnce(page([1], 1, 0))
  ...
  it('does not apply a stale response after the query filter changes', async () => {
    ...
    await act(async () => {
      resolveActive(page([2], 1, 0))
    })
    expect(state!.data).toEqual([{ id: 2 }])

    await act(async () => {
      resolveAll(page([1], 1, 0))
    })
    expect(state!.data).toEqual([{ id: 2 }])
  })

  it('skips loading while disabled', async () => {
    const fetchPage = vi.fn<FetchPage>().mockResolvedValue(page([1], 1, 0))
